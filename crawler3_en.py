...
for tag1, url1 in lv1_list:
    print(tag1)
    sort_url = 'https://api.uza.uz/api/v1/posts/' + url1.lstrip('/') + '?page=1&per_page=20&_f=json&_l=en'
    null = 'null'
    total_num = eval(requests.get(sort_url, timeout=60).text.replace('\/', '/'))['total']
    per_page = 500
    for page_n in range(1, total_num // per_page + 2):
        sort_url = 'https://api.uza.uz/api/v1/posts/category/politics?page={}&per_page={}&_f=json&_l=en'.format(page_n, per_page)
        items = eval(requests.get(sort_url, timeout=60).text.replace('\/', '/'))['data']
        if len(items) > 0:
            for item in items:
                art_tag = tag1
                art_title = item['title'].strip()
                art_url = 'https://uza.uz/en/posts/_{}'.format(item['id'])
                art_tag2 = ''
...
pd.DataFrame.from_dict(data_dict).to_excel('uza.uz_en.xlsx', index=False)
