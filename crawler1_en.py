...
root_url = 'https://www.inform.kz/en'
root_soup = BeautifulSoup(requests.get(root_url, timeout=60).text, 'html.parser')
lv1_list = [[i.text.strip(), urllib.parse.urljoin(root_url, i.find('a')['href'])] for i in root_soup.find('nav').find_all('li')[:-1]]
...
pd.DataFrame.from_dict(data_dict).to_excel('inform.kz_en.xlsx', index=False)
